...
export class CompleteFollowUpUseCase {
  ...
  async execute(id: string, currentUser: AuthenticatedUser, ipAddress?: string): Promise<FollowUpEntity> {
    ...
    await this.accessPolicy.assertCanAccessLead(existing.leadId, currentUser, 'follow_up.complete', ipAddress);

    const followUp = await this.followUps.complete(id);
    if (!followUp) throw new NotFoundException('Follow-up not found.');
    const copy = this.activityDescriptions.followUpCompleted(currentUser);
    await this.activities.create({
      ...
      metadata: { follow_up_id: followUp.id, completed_at: followUp.completedAt?.toISOString(), status: 'completed' },
    });
    return followUp;
  }
}